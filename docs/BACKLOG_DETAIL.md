...
- [ ] **P2 — FIN-108 — Empacotar o app em Docker**
  ...
  **Falta:** Docker não está instalado nesta máquina (nem WSL) — não foi possível rodar `docker compose up` de verdade. Antes de marcar o critério de aceite, rodar num computador com Docker: subir, criar conta e logar, importar um extrato, ativar 2FA, e reiniciar o contêiner (`docker compose restart`) conferindo que os dados continuam lá.
